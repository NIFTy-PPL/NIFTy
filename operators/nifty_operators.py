    ...
    def __init__(self, domain, assign=None, **kwargs):
        ...
        if(assign is None):
            try:
                ...
                assign = np.arange(self.domain.get_dim(
                    split=False), dtype=np.int)
            else:
                assign = self.domain.power_indices.get(
                    "pindex").flatten(order='C')
        else:
            assign = self.domain.enforce_shape(
                assign).astype(np.int).flatten(order='C')
        # build indexing
        self.ind = [np.where(assign == ii)[0] for ii in xrange(
            np.max(assign, axis=None, out=None) + 1) if ii in assign]
...
#        about.infos.cprint("INFO: pseudo unitary projection operator.")
    ...
    def rho(self):
        ...
        if(self.domain.get_dim(split=False) == self.domain.get_dof()):  # no hidden degrees of freedom
            ...
            mof = np.round(np.real(self.domain.calc_weight(self.domain.get_meta_volume(
                total=False), power=-1).flatten(order='C')), decimals=0, out=None).astype(np.int)  # meta degrees of freedom
            for ii in xrange(len(self.ind)):
    ...
    def _multiply(self, x, band=None, bandsup=None, **kwargs):
        ...
        if(band is not None):
            ...
            Px = np.zeros(self.domain.get_dim(split=False),
                          dtype=self.domain.datatype, order='C')
            ...
            Px = np.zeros((len(self.ind), self.domain.get_dim(
                split=False)), dtype=self.target.datatype, order='C')
            x_ = x.val.flatten(order='C')
            for bb in xrange(self.bands()):
                Px[bb][self.ind[bb]] += x_[self.ind[bb]]
            Px = field(self.target, val=Px, target=nested_space(
                [point_space(len(self.ind), datatype=x.target.datatype), x.target]))
            return Px
            ...
            if(isinstance(x, field)):
                if(x_.domain == self.target):
                    ...
                    if(x_.domain.nest[-1] != x.domain):
                        x_ = x_.transform(target=nested_space([point_space(len(
                            self.ind), datatype=x.domain.datatype), x.domain]), overwrite=False)  # ... domain
                    if(x_.target.nest[-1] != x.target):
                        x_.set_target(new_target=nested_space([point_space(
                            len(self.ind), datatype=x.target.datatype), x.target]))  # ... codomain
                else:
                    # repair ...
    ...
    def pseudo_tr(self, x, **kwargs):
        ...
        if(isinstance(x, operator)):
            ...
            x = x.diag(bare=False, domain=self.domain,
                       target=x.domain, var=False, **kwargs)
            if(x is None):
                raise TypeError(about._error.cstring(
            ...
            # compute non-bare diagonal of the vector-vector operator
            # corresponding to the field x
            x = x * np.conjugate(x)
            # weight
        ...
        if(not self.domain.get_dim(split=False) == self.domain.get_dof()):
            x *= np.round(np.real(self.domain.calc_weight(self.domain.get_meta_volume(total=False), power=-
                                                          1).flatten(order='C')), decimals=0, out=None).astype(np.int)  # meta degrees of freedom
