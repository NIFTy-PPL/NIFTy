...
:func:`~nifty.minimization.kl_energies.SampledKLEnergy` within NIFTy8.
...
Furthermore the KL is represented as a stochastic estimate using a set of samples drawn from :math:`\mathcal{Q}_{\bar{\xi}}` which is implemented in NIFTy8 via :func:`~nifty.minimization.kl_energies.SampledKLEnergy` with `minimizer_sampling != None`.
