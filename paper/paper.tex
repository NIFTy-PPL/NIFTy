...
\marginpar{
  ...
  \begin{itemize}
    ...
    \item \href{https://github.com/openjournals/joss-reviews/issues/6382}{\color{linky}{Review}} \ExternalLink
    \item \href{https://github.com/nifty-ppl/nifty}{\color{linky}{Repository}} \ExternalLink
  ...
  {\bfseries Editor:} \href{https://github.com/openjournals/joss-reviews/issues/6382}{Pending
Editor} \ExternalLink \\
  \vspace{1mm}
    {\bfseries Reviewers:}
  \begin{itemize}
...
Reviewers}
  ...
  {\bfseries Submitted:} 19 February 2024\\
...
\(\ln\mathcal{p(\theta|d)} := \mathcal{l}(d, f(\theta)) + \ln\mathcal{p}(\theta) + \mathrm{const}\)
with log-likelihood \(\mathcal{l}\), forward model \(f\) mapping the
parameters of interest \(\theta\) to the data space, and log-prior
\(\ln\mathcal{p(\theta)}\). The goal of the inference is to draw samples
from the posterior \(\mathcal{p}(\theta|d)\).
...
successor geometric Variational Inference (geoVI) Frank (2022). At the
...
information metric and the prior curvature (Frank et al., 2021).
...
JAXopt/Optax-style optimizer class (Blondel et al., 2021; DeepMind et
al., 2020).
...
\(M_p := F_p + \mathbb{1}\) to random input with \(F_p\) denoting the
Fisher metric of the overall likelihood at position \(p\) and
\(\mathbb{1}\) the identity matrix. Within \texttt{NIFTy.re}, the Fisher
