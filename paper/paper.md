...
By virtue of being written in JAX, \texttt{NIFTy.re} effortlessly runs on accelerator hardware such as the GPU, vectorizes models whenever possible, just-in-time compiles code for additional performance, and enables new kinds of inference machineries thanks to being able to retrieve higher order derivates.
...
In contrast to many GP libraries, neither \texttt{NIFTy} nor \texttt{NIFTy.re} assume the posterior to be analytically accessible.
Instead, \texttt{NIFTy} nor \texttt{NIFTy.re} try to approximate the true posterior using VI.
...
In addition to stand-alone sampling functions, \texttt{NIFTy.re} also provides tools to configure and execute the alternating Kullback–Leibler divergence optimization and sample adaption at a higher abstractions level.
...
The benchmarks uses one thread on a compute-node with an Intel Xeon Platinum 8358 CPU clocked at 2.60G Hz and an NVIDIA A100 SXM4 80 GB HBM2 GPU.
