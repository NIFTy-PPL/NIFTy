...
def _get_cov_from_loc(kernel=None, cov_from_loc=None) -> Callable[[NDARRAY, NDARRAY], NDARRAY]:
    if cov_from_loc is None and callable(kernel):

        def cov_from_loc_sngl(x, y):
            return kernel(jnp.linalg.norm(x - y))

        cov_from_loc = vmap(vmap(cov_from_loc_sngl, in_axes=(None, 0)), in_axes=(0, None))
    else:
...
def layer_refinement_matrices(
    ...
    *,
    ...
    if _with_zeros:
        ...
        olf_wgt_sphere = jnp.where(r <= r_cutoff, 1.,
    ...
    fine_kernel = jnp.where(jnp.all(jnp.diag(fine_kernel) > 0.), fine_kernel, fine_kernel_fallback)
...
def refinement_matrices(
    ...
    **kwargs,
    ...
    if shape0.shape != distances0.shape:
        ve = (f"shape of `shape0` {shape0.shape} is incompatible with"
    ...
    if _fine_strategy == "jump":
        dist_by_depth = distances0 / _fine_size**jnp.arange(0, depth).reshape(-1, 1)
    elif _fine_strategy == "extend":
    ...
    olaf = partial(
        ...
        **kwargs)
    opt_lin_filter, kernel_sqrt = vmap(olaf, in_axes=0, out_axes=(0, 0))(dist_by_depth)
...
def refine_conv_general(
    ...
    if olf.shape[:-2] != fine_kernel_sqrt.shape[:-2]:
        ve = ("incompatible optimal linear filter (`olf`) and `fine_kernel_sqrt` shapes"
    ...
    if olf.ndim > 2:
        ...
        raise ValueError(ve)
    olf = olf.reshape(irreg_shape + (fsz**ndim,) + (csz,) * (ndim-1) + (1, csz))
    fine_kernel_sqrt = fine_kernel_sqrt.reshape(irreg_shape + (fsz**ndim,) * 2)

    if _fine_strategy == "jump":
        window_strides = (1,) * ndim
        fine_init_shape = tuple(n - (csz-1) for n in coarse_values.shape) + (fsz**ndim,)
        fine_final_shape = tuple(fsz * (n - (csz-1)) for n in coarse_values.shape)
        convolution_slices = list(range(csz))
    elif _fine_strategy == "extend":
        window_strides = (fsz // 2,) * ndim
        fine_init_shape = tuple(
            ceil((n - (csz-1)) / (fsz//2)) for n in coarse_values.shape) + (fsz**ndim,)
        fine_final_shape = tuple(fsz * ceil((n - (csz-1)) / (fsz//2)) for n in coarse_values.shape)
    ...
    # PyTorch)
        dimension_numbers=(f"N{dim_names}C", f"O{dim_names}I", f"N{dim_names}C"),
        precision=precision,
    )

    c_shp_n1 = coarse_values.shape[-1]
    c_slc_shp = (1,)
    c_slc_shp += tuple(c if i == 1 else csz for i, c in zip(irreg_shape, coarse_values.shape[:-1]))
    c_slc_shp += (-1, csz)
    ...
    irreg_indices = jnp.stack(
        jnp.meshgrid(
            *[jnp.arange(sz) if sz != 1 else jnp.array([PLC]) for sz in irreg_shape],
            indexing="ij"),
        axis=-1)

    def single_refinement_step(i, fine: jnp.ndarray) -> jnp.ndarray:
        irreg_idx = jnp.unravel_index(i, irreg_indices.shape[:-1])
        _assert(len(irreg_shape) == len(irreg_indices[irreg_idx]) == len(window_strides))
        fine_init_idx = tuple(idx if sz != 1 else slice(None)
                              for sz, idx in zip(irreg_shape, irreg_indices[irreg_idx]))
        # Make JAX/XLA happy with `dynamic_slice`
        coarse_idx = tuple((ws * idx, csz) if sz != 1 else (0, cend) for ws, sz, idx, cend in zip(
            window_strides, irreg_shape, irreg_indices[irreg_idx], coarse_values.shape))
        ...
        olf_at_i = jnp.squeeze(
            olf[fine_init_idx], axis=tuple(range(sum(i == 1 for i in irreg_shape))))
            ...
            for i_f, i_c in enumerate(convolution_slices):
                c = conv(
                    coarse_idx_select(coarse_values)[..., i_c:c_shp_n1 -
                                                     (c_shp_n1-i_c) % csz].reshape(c_slc_shp),
                    olf_at_i)[0]
                c = jnp.squeeze(c, axis=tuple(a for a, i in enumerate(irreg_shape) if i != 1))
                toti = fine_init_idx[:-1] + (slice(i_f, None, csz),)
                fine = fine.at[toti].set(c)
        else:
            _assert(not isinstance(fine_init_idx[-1], slice) and fine_init_idx[-1].ndim == 0)
            c = conv(coarse_idx_select(coarse_values).reshape(c_slc_shp), olf_at_i)[0]
            c = jnp.squeeze(c, axis=tuple(a for a, i in enumerate(irreg_shape) if i != 1))
            fine = fine.at[fine_init_idx].set(c)

        return fine

    fine = fori_loop(0, np.prod(irreg_indices.shape[:-1]), single_refinement_step, fine)
    ...
    rm_axs = tuple(
        ax for ax, i in enumerate(m.shape[len(irreg_shape):], len(irreg_shape)) if i == 1)
...
def refine_slice(
    ...
    def matmul_with_window_into(x, y, idx):
        return jnp.tensordot(
            x, dynamic_slice(y, idx, slice_sizes=(csz,) * ndim), axes=ndim, precision=precision)
    ...
    cv_idx = np.mgrid[tuple(
        slice(None, sz - csz + 1, ws) for sz, ws in zip(coarse_values.shape, window_strides))]
...
def refine_conv(coarse_values, excitations, olf, fine_kernel_sqrt, precision=None):
    fine_m = vmap(
        partial(jnp.convolve, mode="valid", precision=precision), in_axes=(None, 0),
        out_axes=0)(coarse_values, olf[::-1])
    fine_m = jnp.moveaxis(fine_m, (0,), (1,))
    fine_std = vmap(
        jnp.matmul, in_axes=(None, 0))(fine_kernel_sqrt,
                                       excitations.reshape(-1, fine_kernel_sqrt.shape[-1]))

    return (fine_m + fine_std).ravel()


def refine_loop(coarse_values, excitations, olf, fine_kernel_sqrt, precision=None):
    fine_m = [jnp.convolve(coarse_values, o, mode="valid", precision=precision) for o in olf[::-1]]
...
def refine_vmap(coarse_values, excitations, olf, fine_kernel_sqrt, precision=None):
    sh0 = coarse_values.shape[0]
    conv = vmap(partial(jnp.matmul, precision=precision), in_axes=(None, 0), out_axes=0)
    fine_m = jnp.zeros((coarse_values.size - 2, 2))
    fine_m = fine_m.at[0::3].set(conv(olf, coarse_values[:sh0 - sh0%3].reshape(-1, 3)))
    fine_m = fine_m.at[1::3].set(conv(olf, coarse_values[1:sh0 - (sh0-1) % 3].reshape(-1, 3)))
    fine_m = fine_m.at[2::3].set(conv(olf, coarse_values[2:sh0 - (sh0-2) % 3].reshape(-1, 3)))
