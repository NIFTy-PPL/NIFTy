...
def leapfrog_step(
    ...
    momentum_halfstep = (momentum - (step_size/2.) * potential_energy_gradient(position))

    position_fullstep = position + step_size * kinetic_energy_gradient(
        inverse_mass_matrix, momentum_halfstep)

    momentum_fullstep = (
        momentum_halfstep - (step_size/2.) * potential_energy_gradient(position_fullstep))
...
def generate_hmc_acc_rej(*, key, initial_qp, potential_energy, kinetic_energy, inverse_mass_matrix,
                         stepper, num_steps, step_size,
                         max_energy_difference) -> AcceptedAndRejected:
    """
    ...
    new_qp = fori_loop(
        lower=0, upper=num_steps, body_fun=lambda _, args: loop_body(args), init_val=initial_qp)
...
def generate_nuts_tree(initial_qp,
                       key,
                       step_size,
                       max_tree_depth,
                       ...
                       max_energy_difference: Union[jnp.ndarray, float] = jnp.inf) -> Tree:
    """Generate a sample given the initial position.
    ...
    initial_neg_energy = -total_energy_of_qp(initial_qp, potential_energy,
...
def count_trailing_ones(n):
    ...
    _, trailing_ones_count = while_loop(lambda nc: (nc[0] & 1) != 0, lambda nc:
                                        (nc[0] >> 1, nc[1] + 1), (n, 0))
...
def is_euclidean_uturn(qp_left, qp_right):
    ...
    """
    return ((qp_right.momentum.dot(qp_right.position - qp_left.position) < 0.) &
            (qp_left.momentum.dot(qp_left.position - qp_right.position) < 0.))


# Essentially algorithm 2 from https://arxiv.org/pdf/1912.11554.pdf
def iterative_build_tree(key, initial_tree, step_size, go_right, stepper, potential_energy,
    ...
    S = tree_util.tree_map(
        lambda proto: jnp.empty_like(proto, shape=(max_tree_depth,) + jnp.shape(proto)), z)

    z = stepper(jnp.where(go_right, 1., -1.) * step_size, inverse_mass_matrix, z)
    neg_energy = -total_energy_of_qp(z, potential_energy,
                                     partial(kinetic_energy, inverse_mass_matrix))
    ...
    def amend_incomplete_tree(state):
        ...
        S, turning = cond(pred=n % 2 == 0, true_fun=_even_fun, false_fun=_odd_fun, operand=S)
...
def add_single_qp_to_tree(key, tree, qp, go_right, potential_energy, kinetic_energy,
    ...
    neg_energy = -total_energy_of_qp(qp, potential_energy,
                                     partial(kinetic_energy, inverse_mass_matrix))
    diverging = jnp.abs(neg_energy - initial_neg_energy) > max_energy_difference
    # ln(e^-H_1 + e^-H_2)
...
def merge_trees(key, current_subtree, new_subtree, go_right, bias_transition):
    ...
    new_sample = select(
        random.bernoulli(key, transition_probability), new_subtree.proposal_candidate,
