...
def refine_conv_general(
    ...
    if _fine_strategy == "jump":
        ...
        fine_init_shape = tuple(n - (csz - 1) for n in coarse_values.shape) + (
            fsz**ndim,
        )
        ...
        fine_final_shape = tuple(
            fsz * ceil((n - (csz - 1)) / (fsz // 2)) for n in coarse_values.shape
        )
    ...
    c_slc_shp = (1,)
    c_slc_shp += tuple(
        c if i == 1 else csz for i, c in zip(irreg_shape, coarse_values.shape[:-1])
    )
    c_slc_shp += (-1, csz)
    ...
    def single_refinement_step(i, fine: jnp.ndarray) -> jnp.ndarray:
        ...
        _assert(
            len(irreg_shape) == len(irreg_indices[irreg_idx]) == len(window_strides)
        )
        fine_init_idx = tuple(
        ...
        if irreg_shape[-1] == 1 and fine_init_shape[-1] != 1:
            ...
            for i_f, i_c in enumerate(convolution_slices):
                c = conv(
                    coarse_idx_select(coarse_values)[
                        ..., i_c : c_shp_n1 - (c_shp_n1 - i_c) % csz
                    ].reshape(c_slc_shp),
                    olf_at_i,
                )[0]
                c = jnp.squeeze(
            ...
            _assert(
                not isinstance(fine_init_idx[-1], slice) and fine_init_idx[-1].ndim == 0
            )
    ...
    rm_axs = tuple(
        ax
        for ax, i in enumerate(m.shape[len(irreg_shape) :], len(irreg_shape))
        if i == 1
    )
