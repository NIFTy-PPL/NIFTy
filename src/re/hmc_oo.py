...
def _parse_diag_mass_matrix(mass_matrix, position_proto: Q) -> Q:
    ...
    elif tree_util.tree_structure(mass_matrix) == tree_util.tree_structure(position_proto):
        shape_match_tree = tree_util.tree_map(lambda a1, a2: jnp.shape(a1) == jnp.shape(a2),
...
class _Sampler:
    ...
    def generate_n_samples(self,
                           ...
                           num_samples,
                           *,
                           save_intermediates: bool = False) -> Tuple[Chain, Tuple[Any, Q]]:
...
class NUTSChain(_Sampler):
    ...
    def init_chain(num_samples: int, position_proto, save_intermediates: bool) -> Chain:
        samples = tree_util.tree_map(
            lambda arr: jnp.zeros_like(arr, shape=(num_samples,) + jnp.shape(arr)), position_proto)
        ...
        if save_intermediates:
            ...
            trees = tree_util.tree_map(
                lambda leaf: jnp.zeros_like(leaf, shape=(num_samples,) + jnp.shape(leaf)),
                _tree_proto)
    ...
    def update_chain(chain: Chain, idx: Union[jnp.ndarray, int], tree: Tree) -> Chain:
        ...
        tree_acceptance = jnp.where(num_proposals > 0, tree.cumulative_acceptance / num_proposals,
                                    0.)
        ...
        chain = chain._replace(
            samples=samples, divergences=divergences, acceptance=acceptance, depths=depths)
...
class HMCChain(_Sampler):
    ...
    @staticmethod
    def update_chain(chain: Chain, idx: Union[jnp.ndarray, int],
                     acc_rej: AcceptedAndRejected) -> Chain:
