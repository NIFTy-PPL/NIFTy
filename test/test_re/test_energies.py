...
lh_init_approx = (
    (jft.VariableCovarianceGaussian, {
     ...
     (random.normal(key, shape=shape), 1. / jnp.exp(random.normal(key, shape=shape)))),
    (jft.VariableCovarianceStudentT, {
        "data": random.normal,
        "dof": random.exponential
    }, lambda key, shape:
     (random.normal(key, shape=shape), jnp.exp(1. + random.normal(key, shape=shape)))))
...
def test_left_sqrt_metric_vs_metric_consistency(seed, shape, lh_init):
    ...
    init_kwargs = {k: method(key=sk, shape=shape) for (k, method), sk in zip(draw.items(), subkeys)}
    ...
    lh_mini = jft.Likelihood(energy, left_sqrt_metric=lsm, lsm_tangents_shape=lsm_shp)
...
def test_transformation_vs_left_sqrt_metric_consistency(seed, shape, lh_init):
    ...
    for _ in range(N_TRIES):
        ...
        t = rng_method(sk.pop(), shape=shape)
        assert_allclose(
            lh.left_sqrt_metric(p, t), lh_mini.left_sqrt_metric(p, t), rtol=rtol, atol=atol)
        assert_allclose(lh.metric(p, t), lh_mini.metric(p, t), rtol=rtol, atol=atol)
