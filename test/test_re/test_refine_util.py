...
def test_shape_translations(shape0, depth, _coarse_size, _fine_size, _fine_strategy):
    kwargs = {
    ...
    def cf(shape0, xi):
        chart = refine_chart.CoordinateChart(
            shape0=shape0, depth=depth, distances0=(1.,) * len(shape0), **kwargs)
        return refine_chart.RefinementField.apply(xi, chart=chart, kernel=lambda x: x)
    ...
    dom_pred = refine_util.get_refinement_shapewithdtype(shape0_pred, depth, **kwargs)
...
def test_gauss_kl(seed, n_resamples=100):
    ...
    for _ in range(n_resamples):
        ...
        np.testing.assert_allclose(refine_util.gauss_kl(m_t, m_t), 0., atol=1e-11)
        kl_rhs_scl = 0.5 * d * (np.log(scl) + 1./scl - 1.)
        np.testing.assert_allclose(kl_rhs_scl, refine_util.gauss_kl(m_t, scl * m_t), rtol=1e-11)
        kl_lhs_scl = 0.5 * d * (-np.log(scl) + scl - 1.)
        np.testing.assert_allclose(kl_lhs_scl, refine_util.gauss_kl(scl * m_t, m_t), rtol=1e-10)
