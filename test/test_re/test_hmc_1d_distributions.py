...
def test_moment_consistency(distribution, plot=False):
    ...
    if plot is True:
        ...
        axs.flat[1].hist(
            chain.depths, bins=jnp.arange(max_tree_depth + 1), density=True, histtype="step")
    ...
    dist_moms_non_central = jnp.array([scipy_dist.moment(i) for i in [1, 2, 3, 4, 5, 6]])
    dist_moms_central = mnc2mc(dist_moms_non_central, wmean=True)

    for i, (smpl_mom, dist_mom) in enumerate(zip(sample_moms_central, dist_moms_central), start=1):
        msg = (f"{name} (moment {i}) :: sampled: {smpl_mom:+.2e}"
        ...
        if i in moments_tol and test:
            assert_allclose(dist_mom, smpl_mom, **dict(zip(("rtol", "atol"), moments_tol[i])))
