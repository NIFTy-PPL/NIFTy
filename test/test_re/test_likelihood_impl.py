...
def random_draw(key, shape, dtype, method):
    ...
    swd = jax.tree_map(lambda x: jft.ShapeWithDtype(x, dtype), shape, is_leaf=_isleaf)
...
def test_gaussian_vs_vcgaussian_consistency(seed, shape):
    ...
    inv_std = 1.0 / jax.tree_map(jnp.exp, 1.0 + inv_std)
    ...
    jax.tree_map(partial(assert_allclose, rtol=rtol, atol=atol), diff_g, diff_vcg)
...
def test_studt_vs_vcstudt_consistency(seed, shape):
    ...
    jax.tree_map(partial(assert_allclose, rtol=rtol, atol=atol), diff_t, diff_vct)
...
def test_transformation_vs_sqrt_metric_consistency(seed, shape, lh_init):
    ...
    for _ in range(N_TRIES):
        ...
        t = latent_init(sk.pop(), shape=shape)
        jax.tree_map(
            aallclose, lh.left_sqrt_metric(p, t), lh_mini.left_sqrt_metric(p, t)
        )
        jax.tree_map(
            aallclose, lh.right_sqrt_metric(p, t), lh_mini.right_sqrt_metric(p, t)
        )
...
def test_residuals_allfinite(seed, shape, lh_init):
    ...
    for _ in range(N_TRIES):
        ...
        assert jax.tree_util.tree_reduce(lambda a, b: a * b, jax.tree_map(allfinite, r))
...
def test_nifty_vcgaussian_vs_niftyre_vcgaussian_consistency(seed, iscomplex):
    ...
    def random_jft_ift_field(key):
        ...
        inp_nft = ift.MultiField(
            op.domain, jax.tree_map(ift.Field, op.domain.values(), tuple(inp.values()))
        )
        return inp, inp_nft
