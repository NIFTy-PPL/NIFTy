...
def test_decay_ornstein_uhlenbeck(x0, dts, gamma):
    ...
    gp = jft.OrnsteinUhlenbeckProcess(1.0, gamma, dts, x0=x0)
...
def test_const_wiener(x0, dts, seed):
    gp = jft.WienerProcess(x0, 1.0, dts)
    myres = gp(jft.zeros_like(gp.domain))
    ...
    amp = np.sqrt(dts)
    res2 = jft.gauss_markov.scalar_gauss_markov_process(p["wp"], x0, 1.0, amp)
...
def test_drift_integrated_wiener(x0, dts):
    ...
    ts[1:] = np.cumsum(dts)
    res = x0[1] * ts + x0[0]
    gp = jft.IntegratedWienerProcess(x0, 1.0, dts)
    myres = gp(jft.zeros_like(gp.domain))
...
@pmp("x0", [1.0, (0.0, 1.5), jft.UniformPrior(-1.0, 1.0, name="x0")])
@pmp("sigma", [0.3, (1.0, 0.8), jft.UniformPrior(0.1, 1.1, name="sigma")])
@pmp(
    "name",
    [
        "wiener",
...
@pmp("x0", [None, 1.0, (0.0, 1.5), jft.UniformPrior(-1.0, 1.0, name="x0")])
@pmp("sigma", [0.3, (1.0, 0.8), jft.UniformPrior(0.1, 1.1, name="sigma")])
@pmp(
    "name",
    [
