...
def matern_kernel(distance, scale, cutoff, dof):
    ...
    return scale**2 * 2**(1 - dof) / jnp.exp(gammaln(dof)) * (reg_dist)**dof * kv(dof, reg_dist)
...
def test_refinement_1d(seed, dist, kernel=kernel):
    ...
    refs = (refine.refine_conv, refine.refine_conv_general, refine.refine_loop, refine.refine_vmap,
            refine.refine_loop, refine.refine_slice)
    cov_from_loc = refine._get_cov_from_loc(kernel=kernel)
    olf, fine_kernel_sqrt = refine.layer_refinement_matrices(dist, kernel)
    ...
    aallclose = partial(assert_allclose, desired=fine_reference, rtol=6 * eps, atol=60 * eps)
    for ref in refs:
...
def test_refinement_nd_cross_consistency(seed,
                                         dist,
                                         _coarse_size,
                                         _fine_size,
                                         _fine_strategy,
    ...
    chart = refine_chart.CoordinateChart(min_shape, depth=depth, distances=dist, **kwargs)
    rfm = refine_chart.RefinementField(chart).matrices(kernel)
    xi = jft.random_like(random.PRNGKey(seed), refine_chart.RefinementField(chart).shapewithdtype)
...
@pmp("_fine_size", (2, 4))
@pmp("_fine_strategy", ("jump", "extend"))
def test_refinement_covariance(dist, _coarse_size, _fine_size, _fine_strategy, kernel=kernel):
    ...
    cf = refine_chart.RefinementField(
        ...
        kernel=kernel)
    exc_shp = [jft.ShapeWithDtype((_coarse_size,) * ndim), jft.ShapeWithDtype((_fine_size,) * ndim)]
    ...
    cov_empirical = jax.vmap(
        lambda idx: cf_cf_T(probe.at[tuple(idx)].set(1.)).ravel(), in_axes=1, out_axes=-1)(
            indices)
...
@pmp("_fine_size", (2, 4))
@pmp("_fine_strategy", ("jump", "extend"))
def test_chart_pixel_refinement_matrices_consistency(dist,
                                                     _coarse_size,
                                                     _fine_size,
                                                     _fine_strategy,
                                                     kernel=kernel):
    ...
    olf_classical, ks_classical = refine.layer_refinement_matrices(distances, kernel, **kwargs)
...
@pmp("_fine_size", (2, 4))
@pmp("_fine_strategy", ("jump", "extend"))
def test_chart_refinement_matrices_consistency(dist,
                                               _coarse_size,
                                               _fine_size,
                                               _fine_strategy,
                                               kernel=kernel):
    ...
    cc = refine_chart.CoordinateChart((12,) * ndim, depth=depth, distances=distances, **kwargs)
    ...
    cc_irreg = refine_chart.CoordinateChart(
        ...
        **kwargs)
    refinement_irreg = refine_chart.RefinementField(cc_irreg).matrices(kernel=kernel)

    _, (cov_sqrt0, _) = refine.refinement_matrices(cc.shape0, 0, cc.distances0, kernel, **kwargs)
    ...
    for lvl in range(depth):
        ...
        olf_irreg, ks_irreg = refinement_irreg.filter[lvl], refinement_irreg.propagator_sqrt[lvl]
...
def test_refinement_irregular_regular_consistency(seed,
                                                  dist,
                                                  _coarse_size,
                                                  _fine_size,
                                                  _fine_strategy,
    ...
    refined = _refine(fn1, exc, refinement.filter[-1], refinement.propagator_sqrt[-1], **kwargs)
    refined_irreg = _refine(fn1, exc, refinement_irreg.filter[-1],
