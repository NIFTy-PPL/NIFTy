...
def generate_data(global_shape, dtype, distribution_strategy,
    ...
    if distribution_strategy in global_distribution_strategies:
        ...
        if rank%2 == 1:
            ...
            number_of_extras = global_shape[0]-local_shape[0]*np.ceil(size/2.)
...
###############################################################################
    ...
    if size > 1:
        ...
        (None, (2,2), np.dtype('int') if (rank == 0) else np.dtype('float'),
             None, None),],
            global_distribution_strategies),
        testcase_func_name=custom_name_func)
        def test_failed_init_on_unsufficient_parameters_only_mpi(self,
                (global_data, global_shape, dtype, local_data, local_shape),
                    distribution_strategy):
            assert_raises(ValueError,
                          lambda: distributed_data_object(
...
###############################################################################
    ...
    if size > 1:
        @parameterized.expand(
            ...
            (None, None, None, np.arange(2*rank).reshape((2,rank)), None),
            (None, None, np.dtype('int') if (rank == 0) else np.dtype('float'),
                 None, (2,2))],
            local_distribution_strategies),
        testcase_func_name=custom_name_func)
        def test_failed_init_on_unsufficient_parameters_only_mpi(self,
...
###############################################################################
    ...
    def test_set_from_non_local_keys(self, distribution_strategy):
        ...
        local_data_update_list = map(lambda z: distributed_data_object(z,
                                    distribution_strategy = 'equal'),
...
###############################################################################
    ...
    def test_apply_generator(self, distribution_strategy):
        ...
        (a, obj) = generate_data(global_shape, start_dtype,
                                     distribution_strategy)
        generator = lambda shape: np.ones(shape = shape, dtype=generator_dtype)
...
class Test_unary_and_binary_operations(unittest.TestCase):
    ...
    itertools.product(['__pos__', '__neg__', '__abs__'],
            [np.dtype('int'), np.dtype('complex64'), np.dtype('complex128')],
            all_distribution_strategies),  testcase_func_name=custom_name_func)
...
class Test_contractions(unittest.TestCase):
    @parameterized.expand(
    itertools.product([np.dtype('int'),np.dtype('float'), np.dtype('complex')],
                       all_distribution_strategies),
                testcase_func_name=custom_name_func)
...
###############################################################################
    ...
    def test_is_real_is_complex(self, distribution_strategy):
        ...
        if stored_distribution_strategy is not None:
            (b, p) = generate_data(global_shape, np.dtype('complex'),
