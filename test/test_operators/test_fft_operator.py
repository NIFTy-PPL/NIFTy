...
def test_fft2D(dim1, dim2, d1, d2, dtype, op, nthreads):
    ...
    b = ift.RGSpace([dim1, dim2], distances=[1. / (dim1*d1), 1. / (dim2*d2)], harmonic=True)
...
def test_composed_fft(index, dtype, op):
    tol = _get_rtol(dtype)
    a = [a1, a2, a3] = [ift.RGSpace((32,)), ift.RGSpace((4, 4)), ift.RGSpace((5, 6))]
    fft = op(domain=a, space=index)

    inp = ift.Field.from_random(
        domain=(a1, a2, a3), random_type='normal', dtype=dtype, std=7, mean=3)
    out = fft.inverse_times(fft.times(inp))
    assert_allclose(inp.val, out.val, rtol=tol, atol=tol)
