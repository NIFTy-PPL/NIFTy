...
def test_integration_operator():
    ...
    res1 = f1.integrate()
    res2 = op1(f1)
    res3 = (f1.asnumpy()*x1.dvol).sum()
    assert_allclose(res1.asnumpy(), res2.asnumpy())
    assert_allclose(res1.asnumpy(), res3)
    res4 = f2.integrate()
    res5 = op2(f2)
    res6 = (f2.asnumpy()*x1.dvol*x2.dvol).sum()
    assert_allclose(res4.asnumpy(), res5.asnumpy())
    assert_allclose(res4.asnumpy(), res6)
    res7 = f2.integrate(spaces=1)
    res8 = op3(f2)
    res9 = (f2.asnumpy()*x2.dvol).sum(axis=(1, 2))
    assert_allclose(res7.asnumpy(), res8.asnumpy())
    assert_allclose(res7.asnumpy(), res9)
