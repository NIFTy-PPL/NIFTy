...
from .num.lanczos import _slq_gauss_radau
...
def estimate_evidence_lower_bound(
    ...
    *,
    ...
    trace_log_method : {"eigsh", "slq"}
        ...
        "slq" uses exact eigenvalues plus stochastic Lanczos Gauss quadrature
        for the remainder. Radau diagnostics are computed only when
    ...
    use_radau_as_bound : bool
        ...
        `lower_error`. At least one exact eigenvalue is required to provide an
        upper spectral endpoint. The result is not guaranteed to be a rigorous
        bound for arbitrary `f` (including log/log1p) unless the corresponding
        conditions for Gauss–Radau bounds are met. Default is False, in which
    ...
    analytic_prior_term : bool
        ...
        exact eigenvalues plus (if needed) a centered SLQ remainder using
        :math:`f(x)=1/x-1` (signal space) or
        :math:`f(x)=1/(1+x)-1` (data space). The removed identity contribution
    ...
    stats : dict
        ...
        - `elbo_lw`: `elbo_mean - elbo_std - lower_error` (one-sigma lower estimate).
        - `lower_error`: estimate of the residual trace-log uncertainty.
          ...
          it is a one-sigma stochastic uncertainty based on the SLQ standard
          error of the remaining trace-log, not a rigorous bound.
          If `analytic_prior_term=True`, an additional term from the SLQ
          standard error of the trace-inverse estimate is included.
        - `trace_inv_exact`, `trace_inv_slq`, `trace_inv_const`, `trace_inv_se`,
          `trace_inv_total`, `prior_mean_sq`, `prior_term`: returned when
        ...
        stats.update(
            {
                ...
                "trace_inv_const": float(trace_inv_const),
