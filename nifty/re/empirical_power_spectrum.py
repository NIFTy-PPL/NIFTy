...
def compute_empirical_power_spectrum(
    ...
    """Computes the average empirical power spectrum of a field across the given axes
    ...
    axes: int, Tuple[int], None
        Axes over which to compute the power spectrum. If None, all axes are consumed
    use_window: bool
        Whether to apply a Hann window to the field along the consumed axes before computing the power.
        Required if the field does not have periodic boundaries.
    n_bins: Optional[int]
    ...
    axis_lengths = jnp.array([field.shape[ax] * dist_tuple[i] for i, ax in enumerate(axes)])
    ...
    ks = [jnp.fft.fftfreq(field.shape[ax], d=dist_tuple[i]) * 2 * jnp.pi 
    ...
    def average_power_per_k_bin(power_vals):
        bin_sum = jax.ops.segment_sum(power_vals.flatten(), indices, num_segments=n_bins)
