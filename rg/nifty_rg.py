...
#            x *= self.get_dim(split=False)
    ...
    def calc_smooth(self, x, sigma=0, codomain=None):
        ...
        if codomain != None:
            ...
            if self.harmonic == False and codomain.fourier == False:
                raise ValueError(about._errors.cstring(
    ...
    def calc_power(self, x, **kwargs):
        ...
        # TODO: Use d2o.bincount in order to simplify this
            ...
            # the pindex and the kindex must have the same distribution
            # strategy
    ...
    def get_plot_new(self, x, title="", vmin=None, vmax=None, power=None, unit="",
        ...
        if(power):
            ...
            if(naxes == 1):
                ...
                xaxes = (np.arange(
                    self.para[0], dtype=np.int) + self.para[2] * (self.para[0] // 2)) * self.vol
                if(vmin is None):
                    if(np.iscomplexobj(x)):
                ...
                if issubclass(x.dtype.type, np.complexfloating):
                    ...
                    s_ = np.array([self.para[1] * self.vol[1] / np.max(self.para[:naxes] * self.vol, axis=None, out=None), self.para[
                                  0] * self.vol[0] / np.max(self.para[:naxes] * self.vol, axis=None, out=None) * (1.0 + 0.159 * bool(cbar))])
                    ...
                    xaxes = (np.arange(self.para[
                             1] + 1, dtype=np.int) - 0.5 + self.para[4] * (self.para[1] // 2)) * self.vol[1]
                    yaxes = (np.arange(self.para[
                             0] + 1, dtype=np.int) - 0.5 + self.para[3] * (self.para[0] // 2)) * self.vol[0]
                    if(norm == "log"):
                        n_ = ln(vmin=vmin, vmax=vmax)
                    else:
                        n_ = None
...
#        return global_klength
    ...
    def _bin_power_indices(self, index_dict, **kwargs):
        ...
        # equal binning
            ...
            binbounds = np.r_[
                0.5 * (3 * k[1] - k[2]), 0.5 * (k[1] + k[2]) + dk * np.arange(nbin - 2)]
            if(log):
        ...
        for ii in range(len(reorder)):
            if(rho_[reorder[ii]] == 0):
                ...
                kindex_[reorder[ii]] = (kindex_[reorder[
                                        ii]] * rho_[reorder[ii]] + kindex[ii] * rho[ii]) / (rho_[reorder[ii]] + rho[ii])
                rho_[reorder[ii]] += rho[ii]
