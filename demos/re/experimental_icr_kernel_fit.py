...
from nifty8.re.refine.util import get_cov_from_loc, refinement_matrices, RefinementMatrices
...
class Kerparams(jft.Model):
    def __init__(self):
        self._a = jft.LogNormalPrior(1., 0.5, name='a')
        self._sig = jft.LogNormalPrior(1., 0.5, name='sig')
...
samples, state = jft.optimize_kl(
    ...
    kl_kwargs=dict(
        minimize_kwargs=dict(
            name="M", absdelta=delta * jft.size(lh.domain),
            cg_kwargs=dict(name="MCG"), maxiter=20
        )
    ),
...
j = j.at[signal_response._pix].set(1.)
...
for i, (g, r, ss, jj) in enumerate(zip(synth_density.T, m.T, sa.T, j.T)):
    ...
    hp.mollview(
        ...
        fig=fig.number,
        sub=(4, synth_density.shape[1], i+1+3*synth_density.shape[1]),  # (nrows, ncols, index)
        cmap='viridis',
        nest=True,
        notext=True,
        cbar=False
...
fig.suptitle((f'a: {gtparams['a']:.2f} vs {mean['a']:.2f} pm {std['a']:.2f}' +
