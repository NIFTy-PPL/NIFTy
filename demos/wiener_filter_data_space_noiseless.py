...
class CustomResponse(ift.LinearOperator):
    ...
    def __init__(self, domain, data_points):
        ...
        data_shape = ift.Field.zeros(domain).val[data_points].shape
        self._target = ift.DomainTuple.make(ift.UnstructuredDomain(data_shape))

    def _times(self, x):
        d = ift.Field.zeros(self._target)
        d.val[()] += x.val[self._points]
        return d

    def _adjoint_times(self, d):
        x = ift.Field.zeros(self._domain)
        x.val[self._points] += d.val[()]
        return x
...
if __name__ == "__main__":
    ...
    Rx = CustomResponse(s_space, [np.arange(0,N_pixels, 5)[:,np.newaxis], np.arange(0,N_pixels, 2)[np.newaxis,:]])
    ...
    M = (ift.SandwichOperator(R.adjoint, Sh) + N)
    M = ift.InversionEnabler(M, inverter)
    m = Sh(R.adjoint(M.inverse_times(d)))

    # Plotting
    backprojection = Rx.adjoint(d)
    reweighted_backprojection = backprojection / backprojection.max() * HT(sh).max()
